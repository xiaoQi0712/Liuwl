...
//新建
  $('.layui-icon-upload-circle').on('click', function(){
    layer.open({
      ...
      content: ` <form id="atvFrom2" action="cust/addacv" method="post">
    <div class="layui-form-item">
    <input id="hidden4" type="hidden" name="atv_cust_id" value="${cust_id}" >
    <label class="layui-form-label">姓名</label>
    <div class="layui-input-block">
      <input type="text" name="atv_cust_name" required  lay-verify="required" placeholder="请输入姓名" autocomplete="off" class="layui-input">
    </div>
  </div>
      <div class="layui-form-item">
  ...
  <div class="layui-form-item">
    ...
    $('#hidden4').attr('value',atv_cust_id); 
        });
//编辑
  $('.layui-icon-edit').on('click', function(){
    ...
    layer.open({
      ...
      shadeClose: true, //点击遮罩关闭
      content: `
   <form id="atvFrom1" action="cust/updateacv" method="post">        
...
//删除
   $('.layui-icon-delete').on('click', function(){
	   var atv_id = $(this).attr('value'); 
        	 ...
        	 $('.layui-body').load('cust/deleteacv?atv_id='+atv_id);
